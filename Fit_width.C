...
void FitLP () {
	TFile* fin = new TFile("/Users/seul/seul_workspace/f0ana/results/ana_results/InvMassOut_pass6.root","read");
	...
	for (int i=0 ; i<nmult ; i++) {
		...
		for (int j=0 ; j<npt ; j++) {
			...
			//	width
			width[i][j] = fitBinComp[i][j][1]->GetParameter(2);
		}
		...
		TCanvas* cw = new TCanvas("width","cw",800,600);
		// TGraph* gWidth = new TGraph(npt);
		TGraphErrors* gWidth = new TGraphErrors(npt);
		
		// for (int j = 0; j < npt; j++) {
		// 	gWidth->SetPoint(j, (p_min[j]+p_max[j])/2., width[i][j]);
		// }
		for (int j = 0; j < npt; j++) {
		    double pt_mid = (p_min[j] + p_max[j]) / 2.0;
		    double pt_err = (p_max[j] - p_min[j]) / 2.0;
			gWidth->SetPoint(j, pt_mid, width[i][j]);
			gWidth->SetPointError(j, pt_err, 0);
		}
		
		gWidth->SetTitle("Initial f_{0} width");
		gWidth->GetXaxis()->SetTitle("p_{T} (GeV/c)");
		gWidth->GetYaxis()->SetTitle("Width (MeV/c^{2})");
		gWidth->SetMarkerStyle(25);
		gWidth->SetMarkerSize(1.0);
		gWidth->Draw("AP");

		TString fileName_width = Form("plot/width_mult_%d_%d.pdf", m_min[i], m_max[i]);
		cw->SaveAs(fileName_width);
	...
	TFile* fout = new TFile(Form("/Users/seul/seul_workspace/f0ana/results/ana_results/F0IntgrOut.root"),"recreate");
	for (int i=0;i<nmult;i++) {
		hPtStat[i]->Write();
	}
}
